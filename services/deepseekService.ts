...
async function callDeepSeekAPI(prompt: string) {
    ...
    if (!response.ok) {
        ...
        console.error("DeepSeek (OpenRouter) API Error:", errorText);
        throw new Error(`Ошибка API DeepSeek (OpenRouter): ${response.status} ${response.statusText}. ${errorText}`);
    }
...
ИНАЧЕ:
  ...
  // Просто верни точную строку 'Ставки нет'.`
    ...
    try {
        ...
        console.error('DeepSeek API error during single match analysis:', error);
         if (error instanceof Error) {
            throw new Error(`Не удалось сгенерировать детальный анализ через DeepSeek: ${error.message}`);
        }
        throw new Error("Не удалось сгенерировать детальный анализ через DeepSeek: Неизвестная ошибка.");
    }
}
