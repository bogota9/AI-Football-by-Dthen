...
async function callKimiAPI(prompt: string) {
    ...
    if (!response.ok) {
        ...
        console.error("Kimi (OpenRouter) API Error:", errorText);
        throw new Error(`Ошибка API Kimi (OpenRouter): ${response.status} ${response.statusText}. ${errorText}`);
    }
...
ИНАЧЕ:
  ...
  // Просто верни точную строку 'Ставки нет'.`
    ...
    try {
        ...
        console.error('Kimi API error during single match analysis:', error);
         if (error instanceof Error) {
            throw new Error(`Не удалось сгенерировать детальный анализ через Kimi: ${error.message}`);
        }
        throw new Error("Не удалось сгенерировать детальный анализ через Kimi: Неизвестная ошибка.");
    }
}
