...
async function callLlamaAPI(prompt: string) {
    const body: any = {
        messages: [
    ...
    if (!response.ok) {
        ...
        console.error("Llama (OpenRouter) API Error:", errorText);
        throw new Error(`Ошибка API Llama (OpenRouter): ${response.status} ${response.statusText}. ${errorText}`);
    }
...
ИНАЧЕ:
  ...
  // Просто верни точную строку 'Ставки нет'.`
    ...
    try {
        const content = await callLlamaAPI(prompt);
        const result = parseJsonFromText(content);
        return result;
    } catch (error) {
        console.error('Llama API error during single match analysis:', error);
         if (error instanceof Error) {
            throw new Error(`Не удалось сгенерировать детальный анализ через Llama: ${error.message}`);
        }
        throw new Error("Не удалось сгенерировать детальный анализ через Llama: Неизвестная ошибка.");
    }
}
