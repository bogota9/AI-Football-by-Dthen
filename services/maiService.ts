...
async function callMaiAPI(prompt: string) {
    const body: any = {
        messages: [
    ...
    if (!response.ok) {
        ...
        console.error("MAI (OpenRouter) API Error:", errorText);
        throw new Error(`Ошибка API MAI (OpenRouter): ${response.status} ${response.statusText}. ${errorText}`);
    }
...
ИНАЧЕ:
  ...
  // Просто верни точную строку 'Ставки нет'.`
    ...
    try {
        const content = await callMaiAPI(prompt);
        const result = parseJsonFromText(content);
        return result;
    } catch (error) {
        console.error('MAI API error during single match analysis:', error);
         if (error instanceof Error) {
            throw new Error(`Не удалось сгенерировать детальный анализ через MAI: ${error.message}`);
        }
        throw new Error("Не удалось сгенерировать детальный анализ через MAI: Неизвестная ошибка.");
    }
}
