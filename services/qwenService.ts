...
const MODEL = "qwen/qwen-2-72b-instruct";
...
async function callQwenAPI(prompt: string) {
    ...
    if (!response.ok) {
        ...
        console.error("Qwen (OpenRouter) API Error:", errorText);
        throw new Error(`Ошибка API Qwen (OpenRouter): ${response.status} ${response.statusText}. ${errorText}`);
    }
...
ИНАЧЕ:
  ...
  // Просто верни точную строку 'Ставки нет'.`
    ...
    try {
        const content = await callQwenAPI(prompt);
        const result = parseJsonFromText(content);
        return result;
    } catch (error) {
        console.error('Qwen API error during single match analysis:', error);
         if (error instanceof Error) {
            throw new Error(`Не удалось сгенерировать детальный анализ через Qwen: ${error.message}`);
        }
        throw new Error("Не удалось сгенерировать детальный анализ через Qwen: Неизвестная ошибка.");
    }
}
